...
lazy_static! {
    static ref IGNORE_BLOCK_START: AhoCorasick =
        AhoCorasick::new(IGNORE_BLOCK.iter().map(|x| &x.0).copied());
}
...
pub fn extract<'i, I: IndexData + 'i>(
    ...
    while i < lines.len() {
        if let Some(m) = IGNORE_BLOCK_START
            .find_iter(lines[i].line.sanitized())
            .next()
        {
            trailing_context = 0;
        ...
        if let State::Ignoring(a) = state {
            if a.find_iter(lines[i].line.sanitized()).next().is_some() {
